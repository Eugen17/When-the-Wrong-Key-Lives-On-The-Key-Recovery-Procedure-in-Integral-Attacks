...
// ============================================================
// PRESENT parameters / helpers
// ============================================================
enum { BLOCK_SIZE = 64, SBOX_SIZE = 4, NUM_SBOXES = 16 };
static const uint8_t PRESENT_S_INV[16] = {
...
static void init_sbox_inv_lut(void) {
    for (uint32_t val=0; val < (1u<<16); ++val) {
        ...
        uint32_t v = val;
        // process 4 nibbles
        out |= (uint32_t)PRESENT_S_INV[(v >>  0) & 0xF] <<  0;
        out |= (uint32_t)PRESENT_S_INV[(v >>  4) & 0xF] <<  4;
        out |= (uint32_t)PRESENT_S_INV[(v >>  8) & 0xF] <<  8;
        out |= (uint32_t)PRESENT_S_INV[(v >> 12) & 0xF] << 12;
        SBOX_INV_LUT[val] = (uint16_t)out;
    }
}

static void init_p_maps_and_luts(void) {
    for (int i=0;i<64;++i) P_MAP[i] = (i!=63) ? (uint8_t)((i*16)%63) : 63;
    for (int i=0;i<64;++i) P_INV_MAP[P_MAP[i]] = (uint8_t)i;

    // Build P_LUT and P_INV_LUT chunkwise
