...
// RECTANGLE inverse S-box  S^{-1}
// (derived from S(x): 6 5 C A 1 E 7 9 B 0 3 D 8 F 4 2)
static const uint8_t RECTANGLE_S_INV[16] = {
...
static void init_sbox_inv_lut(void) {
    for (uint32_t val=0; val < (1u<<16); ++val) {
        ...
        uint32_t v = val;
        // process 4 nibbles
        out |= (uint32_t)RECTANGLE_S_INV[(v >>  0) & 0xF] <<  0;
        out |= (uint32_t)RECTANGLE_S_INV[(v >>  4) & 0xF] <<  4;
        out |= (uint32_t)RECTANGLE_S_INV[(v >>  8) & 0xF] <<  8;
        out |= (uint32_t)RECTANGLE_S_INV[(v >> 12) & 0xF] << 12;
        SBOX_INV_LUT[val] = (uint16_t)out;
    }
}

static void init_p_maps_and_luts(void) {
    // RECTANGLE-64 bit permutation P64(i)
    static const uint8_t RECTANGLE_P64[64] = {
        12, 17, 62, 3, 16, 21, 2, 7, 20, 25, 6, 11, 24, 29,
             10, 15, 28, 33, 14, 19, 32, 37, 18, 23, 36, 41, 
             22, 27, 40, 45, 26, 31, 44, 49, 30, 35, 48, 53, 
             34, 39, 52, 57, 38, 43, 56, 61, 42, 47, 60, 1, 46, 
             51, 0, 5, 50, 55, 4, 9, 54, 59, 8, 13, 58, 63

    };

    for (int i=0;i<64;++i) {
        P_MAP[i] = RECTANGLE_P64[i];
